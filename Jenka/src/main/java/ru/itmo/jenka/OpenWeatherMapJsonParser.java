...
public class OpenWeatherMapJsonParser {
    private final static String API_CALL_TEMPLATE = "https://api.openweathermap.org/data/2.5/forecast?lat=latitude&lon=longitude&units=metric&APPID=4b886177c23b65a5528e74cfdf2c5e92";
    ...
    public String getReadyForecast(Float lat, Float lon) {
        String result;
        try {
            String jsonRawData = downloadJsonRawData(lat, lon);
            List<String> linesOfForecast = convertRawDataToList(jsonRawData);
            result = String.format("%s:%s%s", lat + " " + lon, System.lineSeparator(), parseForecastDataFromList(linesOfForecast));
        } catch (IllegalArgumentException e) {
            return String.format("Can't find \"%s\" city. Try another one, for example: \"Kyiv\" or \"Moscow\"", lat + " " + lon);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static String downloadJsonRawData(Float lat, Float lon) throws Exception {
        String urlString = API_CALL_TEMPLATE.replaceAll("longitude", String.valueOf(lon)).replaceAll("latitude", String.valueOf(lat));
